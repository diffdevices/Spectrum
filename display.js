...
var _offs = 2;  //everything is translated (x and y) by this and scaled accordingly
...
function lerp(curr, val1, val2){
    return u.map(curr, 0 , 1, val1, val2);
}
...
function onresize(_newW, _newH){
    var newW = _newW - (!script.isWind?_offs*2:0);
    var newH = _newH - (!script.isWind?_offs*2:0);
...
function Filter(_name, type, freq, noOs){
    ...
    this.normalizeQ = function(){
        ...
        var G = Math.pow(wantedAmp, 1/(this.N/2));
        var wf2 = Math.pow(2, wantedBW); //(2^(BW/2))^2
        
        var normFact = wf2/(wf2*wf2 - 2*wf2 + 1);
        this.BPFFact = Math.sqrt(normFact * (1/(G*G) - 1));
        
        var common = wf2*wf2 - 2*wf2 + 1;
        this.BSFFact = Math.sqrt(wf2/(common/(G*G) - common));

        this.Q *= this.isBandpass ? this.BPFFact : this.BSFFact;
    },
...
function Band(n, freq){
    ...
    this.dynamicFreqParams = function(changed){
        ...
        this.dynamicFreq = changed.dynamicFreq;
...
function Spectrum(parent, name, isOn){
    ...
    this.display = function(color){
        ...
        for(var i in this.spectPos){
            ...
            var posX = 
                (i==0) ? -this.dispOffs : 
                (i==this.spectPos.length-1 ? u.fMaxPosI :
                this.spectPos[i][0])
            ;
            var posY = this.spectPos[i][1];
        ...
        mg.set_source_rgba(colors.get(this.name+"Spect"));
        mg.set_line_width(this.parent.lineW);
        mg.stroke();
    };
    ...
    this.ramp = function(index, poY){
        if(!parent.pixDClip) return poY;
        if(typeof this.spectPos[index] == "undefined") return poY;

        return Math.min(poY, this.spectPos[index][1] + parent.pixDClip);
    };
...
var analyzer = {
    ...
    _pixDClip: 6,//pixels per frame, 0 is no deltaclip
    roundCorners: 5,
    frameSkip: 0,
    autoFrameSkip: true,

    timeout: function(){
        this.spectIter("resetSpect");
    },
    ...
    calcBinToPixel: function(){
        ...
        for(var bin=0; bin< this.size/2; bin++){    //real FFT, from 0 to nyq instead of sr
            ...
            this.binToPixel[bin] = u.freqToPos(currFreq);

            this.binToPixelR[bin] = Math.round(u.freqToPos(currFreq)/this.subsampSize) * this.subsampSize;
        }
    },
    ...
    calcPixDClip: function(){
        this.pixDClip = this._pixDClip * (this.frameSkip+1);
    },
...
var grid = {
    ...
    resetFreqScaleLineY: function(){
        ...
        this.freqScaleLineY = Math.floor(height-this.lMarginB) - 0.5;
    },
    display: function(){
...
var background = {
    ...
    drawBorder: function(){
        ...
        mg.rectangle_rounded(-_offs*2,-_offs*2,width+_offs*4, height+_offs*4, rCor, rCor);
        mg.set_source_rgba(colors.get("patcherBg"));
        mg.set_line_width(_offs*4);
        mg.stroke();
...
var u = {
    ...
    midiToFreq: function(noteNum){
        return Math.pow(2, (noteNum-20-49)/12) * 440;
    },
